...
class AbstractTieredCacheTest extends Specification implements RedisTestContainer {
    ...
    static class MyCache extends AbstractTieredCache<String, MyBean> {
    ...
    def 'should get or compute a value with condition' () {
        ...
        when:
        MyBean r1 = cache.getOrCompute(k1, (it)-> new AbstractTieredCache.Pair<MyBean,Duration>(new MyBean(it+'1', it+'2'), TTL))
        then:
        ...
        when:
        MyBean r2 = cache.getOrCompute(k2, (it)-> new AbstractTieredCache.Pair<MyBean,Duration>(new MyBean(it+'3', it+'4'), null))
        then:
    ...
    def 'should throw IllegalArgumentException for null key' () {
        given:
        def store = context.getBean(RedisL2TieredCache)
        def cache = new MyCache(store)

        when:
        cache.put(null, new MyBean('x', 'y'), Duration.ofMinutes(1))

        then:
        thrown(IllegalArgumentException)
    }

    def 'should throw IllegalArgumentException for null value' () {
        given:
        def store = context.getBean(RedisL2TieredCache)
        def cache = new MyCache(store)

        when:
        cache.put('key', null, Duration.ofMinutes(1))
