...
/**
 * Verifies the classloader-safety claim of the lib-data-workqueue metrics design:
 * with micrometer-core excluded from the classpath, AbstractWorkQueue and a
 ...
 *  - Load LocalWorkQueue + TestPlainQueue through this loader and instantiate
 ...
 */
class QueueMetricsClassloaderTest extends Specification {
    ...
    def 'AbstractWorkQueue is loadable and usable without micrometer-core'() {
        given:
        def isolated = new URLClassLoader(classpathWithoutMicrometer(), ClassLoader.platformClassLoader)
        ...
        def instance = subclassCls.getDeclaredConstructor(workQueueIfc).newInstance(target)
