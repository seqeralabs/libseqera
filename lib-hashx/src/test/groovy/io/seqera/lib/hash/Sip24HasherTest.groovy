...
/**
 * Unit tests for Sip24Hasher.
 *
 * @author Paolo Di Tommaso
 */
class Sip24HasherTest extends Specification {
    ...
    def 'should produce consistent hash for same input'() {
        given:
        def hasher1 = new Sip24Hasher()
        def hasher2 = new Sip24Hasher()
    ...
    def 'should be deterministic across multiple calls'() {
        ...
        def results = (1..100).collect {
            new Sip24Hasher().putString('test').putInt(123).toLong()
        }
    ...
    def 'should handle boolean values'() {
        given:
        def hasherTrue = new Sip24Hasher()
        def hasherFalse = new Sip24Hasher()
    ...
    def 'should have low collision rate for sequential strings'() {
        ...
        def hashes = (1..1000).collect { i ->
            new Sip24Hasher().putString("item-$i").toLong()
        }
    ...
    def 'should exhibit avalanche effect for strings'() {
        given:
        def hash1 = new Sip24Hasher().putString('test').toLong()
        def hash2 = new Sip24Hasher().putString('Test').toLong()
